...
2. Toda solicitud explícita de catálogo pasa directamente a **BC - Simulador - Catálogo PDF**, sin preguntar por compra mayorista o por unidades. `catálogo JBL` reúne los productos publicados JBL con stock y foto utilizable; `audífonos JBL` intersecta marca y categoría; `catálogo de audífonos` incluye todas las marcas, agrupadas. Se toleran variantes como `audofnos`. Un modelo/código adicional restringe la selección. Se excluyen ocultos, agotados, fotos genéricas, videos y fotos que no se pueden cargar. El conteo y el manifiesto corresponden a los productos realmente incluidos. Sin filtros, devuelve el enlace general; sin coincidencias, lo informa sin enviar productos ajenos.
...
En el panel, iniciar una sesión nueva y probar saludo, catálogo, producto/código, stock, envíos a Lima/provincia y datos de compra. La ampliación local del 18 de septiembre admite texto, imágenes JPG/PNG/WebP y audio MP3/MP4/OGG/WAV/WebM de hasta 4 MB, con texto opcional. Requiere publicar juntos web y flujos actualizados; todavía no acredita su disponibilidad en el VPS. Revisar el límite de cuerpo de Nginx/n8n (el archivo se codifica en base64) antes de la prueba integrada.

Una captura de TikTok, Facebook o Instagram entra como imagen. No se descarga ni interpreta automáticamente el contenido de un enlace social. El motor combina transcripción y texto en el orden recibido y conserva la imagen correspondiente. Por ahora admite una imagen y un audio por consulta; ante más archivos pide separarlos, sin elegir silenciosamente solo el último. En la etapa de comprobante conserva la foto/documento como evidencia y no lo pasa al identificador de productos.
