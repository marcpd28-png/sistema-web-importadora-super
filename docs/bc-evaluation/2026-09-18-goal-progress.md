...
## Lectura local de códigos visibles en imágenes

- Motor `2efffe7`, publicado y activo en VPS con respaldo. Tras intentar la coincidencia exacta de foto, el endpoint de visión puede leer una etiqueta CODIGO/COD/SKU mediante Tesseract y Pillow ya instalados en el servidor. No usa proveedor de pago ni descarga enlaces del cliente para OCR.
- Dos modos de segmentación deben leer el mismo código completo con confianza mínima por palabra; debe haber un solo candidato y un único producto visible de código exacto. No convierte letras ambiguas en dígitos ni elige entre varios códigos. Límites: 4 MB, 12 millones de píxeles, reducción a 1800 px, dos procesos de hasta cuatro segundos, una ejecución simultánea por proceso Node; si falla, queda disponible el tratamiento anterior. Se puede desactivar con BC_LOCAL_IMAGE_OCR_ENABLED=false.
- Pasaron 71 pruebas, TypeScript, ESLint y build. Integración con PostgreSQL temporal y Tesseract real: imágenes generadas PNG/JPEG, etiquetas con código, varios códigos, texto sin etiqueta, producto inexistente/oculto e imagen dañada. Base temporal eliminada.
- Prueba publicada: JPEG generado con texto de referencia y CODIGO N1321, no una publicación real de red social. El endpoint tardó 539 ms en esta ejecución y devolvió local-tesseract-labeled-code. La misma imagen por simulador→n8n identificó N1321, mostró precio vigente y pidió cantidad. Sin respuestas duplicadas; contacto temporal eliminado. Evidencia: `2026-09-18-ocr-live.json`.
- Alcance pendiente: validar fotos y capturas reales más variadas, códigos pequeños/borrosos, modelos sin SKU, reconocimiento por apariencia y contenido de enlaces/vídeos. La agenda de grupos mixtos aún utiliza coincidencia exacta de foto; falta incorporar este OCR allí. No equivale a comprensión visual general ni completa el objetivo.

## OCR dentro de grupos de imágenes y texto

- Web `9ccf54a`, publicada y activa con respaldo. La agenda ahora intenta OCR local cuando una foto no coincide exactamente con su referencia original. Conserva el mismo criterio del motor: código explícitamente etiquetado, acuerdo entre dos lecturas y coincidencia única en el catálogo visible.
- Máximo dos intentos OCR por grupo para acotar la duración; el resto de referencias no identificadas permanece pendiente de aclaración. Se conservan las rutas especializadas para adjuntos individuales y checkout.
- Pasaron 90 pruebas, TypeScript, ESLint, compilación e integración con PostgreSQL temporal y Tesseract real: cinco mensajes (pago, captura generada con código visible, precio, seis unidades, envío) producen tres solicitudes en orden con la cantidad correcta. Base temporal eliminada.
- Esto amplía el manejo de capturas con código legible; sigue pendiente la validación con publicaciones reales variadas y el reconocimiento por apariencia/modelos sin código explícito.
- Verificación publicada por simulador y n8n: JPEG generado con etiqueta N1321 → pago, precio de dos unidades (258), envío; imagen no identificable → pago, aclaración, envío. Tres solicitudes por cada grupo de cinco mensajes, cero duplicados tardíos y contactos temporales eliminados. Evidencia: `2026-09-18-mixedocr-live.json`.

## Compra de parlante y dron reportada por el usuario

- Reproducción inicial: el mensaje completo se convertía en un solo SEARCH con ambos nombres concatenados y cantidad dos sobre el tema combinado. El catálogo real sí contenía O1008 (parlante, 17 unidades) y O1001 (dron, una unidad) al verificar.
- Web `f0505ba`, publicada y activa con respaldo. La agenda ahora conserva intención de compra por solicitud, separa adiciones explícitas («y también», «además») y elimina frases de compra del nombre sin modificar modelos o colores. La cantidad se asocia a su cláusula/producto; si falta, se pregunta en vez de asumir una unidad.
- La intención se conserva en adiciones entre mensajes del mismo grupo. Una corrección explícita «del parlante quiero 1 unidad» actualiza ese artículo, mantiene la cantidad del dron y se vuelve a responder. Se corrigió además la lectura del singular «unidad».
- No se envía la última selección al checkout de un solo artículo cuando hay varias intenciones de compra. Ambos artículos quedan en la agenda. IMPORTANTE: esto NO implementa todavía un carrito/pedido conjunto ni permite afirmar que la venta completa de varios artículos esté resuelta.
- Pasaron 93 pruebas, TypeScript, ESLint, build e integración PostgreSQL temporal: caso exacto, variantes de compra/adición/cantidad y corrección posterior. La primera integración detectó que una corrección actualizaba la agenda pero no marcaba recognized; corregido y repetido con éxito. Bases temporales eliminadas.
- Prueba publicada simulador→n8n: el mensaje exacto respondió primero O1008 preguntando cantidad y luego O1001 informando stock insuficiente para dos. La versión de cinco mensajes agregó pago y envío en orden. Cero duplicados; dos contactos de prueba eliminados. Evidencia: `2026-09-18-multiintent-live.json`.
- Siguiente alcance necesario: carrito y confirmación de pedido de múltiples artículos; intérprete semántico validado para formulaciones abiertas, anáforas, errores de escritura y negaciones. No hay proveedor de comprensión general configurado y no se afirma comprensión universal ni ausencia permanente de errores.
