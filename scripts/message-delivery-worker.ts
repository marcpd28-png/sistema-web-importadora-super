...
async function main() {
  ...
  const { runMessageWorkerCycle } = await import("../src/lib/message-worker-cycle");
  let stopped = false;
  process.once("SIGTERM", () => { stopped = true; });
  process.once("SIGINT", () => { stopped = true; });
  while (!stopped) {
    await runMessageWorkerCycle([
      { name: "bc", run: () => processBcLivePilot(prisma) },
      { name: "receipts", run: async () => { if (process.env.WHATSAPP_STATUS_SYNC_ENABLED === "true") await reconcileDeliveryEvents(prisma); } },
      { name: "images", run: async () => { if (process.env.MANYCHAT_IMAGE_QUEUE_ENABLED === "true" && process.env.MANYCHAT_IMAGE_FLOW_ENABLED === "true") {
    ...
    ], name => console.error(`[message-worker] ${name} failed; persisted work retained for review/retry.`));
