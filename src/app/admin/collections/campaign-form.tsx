...
export function CampaignForm({ campaign }: { campaign: Campaign }) {
  ...
  return <form action={formAction} className={styles.editor}>
      ...
      <div className={styles.fields}>
        <label className={styles.field}><span className={styles.fieldTitle}>Productos <small>{codeCount} / 500 códigos</small></span><span className={styles.hint} id={`${campaign.slug}-codes-help`}>Pega los códigos exactos, uno por línea. El orden define cómo aparecen.</span><textarea aria-describedby={`${campaign.slug}-codes-help`} name="codes" rows={6} className={styles.codes} value={codes} onChange={event => setCodes(event.target.value)} placeholder={"Ejemplo:\nN2220\nO15-AZUL"} /><span className={styles.hint}>Los códigos se validan al guardar. También puedes separarlos con comas.</span></label>
