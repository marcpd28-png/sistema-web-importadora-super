...
import { identifyCatalogImageNames, type ImageNameText } from "./image-names";
...
export async function identifyCatalogImageCodes(imageUrl: string, visibleCodes: readonly (string | CatalogImageReference)[]): Promise<ImageCodeMatch | null> {
  ...
  try {
    ...
    if (!codes.length) {
      const matches = identifyCatalogImageNames(output.text, visibleCodes.map(product => typeof product === "string" ? { code: product } : product));
      const names = [...new Set(matches.map(match => match.code))];
      return names.length ? { status: names.length === 1 && matches.every(match => match.complete) ? "READY" : "CHOICES", model: "local-catalog-name-multipass", hints: { code: names.join(", "), confidence: Math.min(...matches.map(match => match.confidence)) }, codes: names } : null;
    }
